...
def _get_cffi_session():
    ...
    if _cffi_session is None:
        ...
        with profiler.span("client.cffi_session.init"):
...
class TwitterClient:
    ...
    def fetch_tweet_detail(self, tweet_id, count=20, page_limit=None):
        # type: (str, int, Optional[int]) -> List[Tweet]
    ...
    def fetch_tweet_head(self, tweet_id):
        # type: (str) -> Tweet
        """Fetch only the focal tweet without paginating replies."""
        ...
        if not result:
            raise NotFoundError("Tweet not found: tweet_id=%s" % tweet_id)
    ...
    def _fetch_timeline(self, operation_name, count, get_instructions, extra_variables=None, override_base_variables=False, field_toggles=None, use_post=False, include_promoted=False, start_cursor=None, return_cursor=False, page_limit=None):
        # type: (str, int, Callable[[Any], Any], Optional[Dict[str, Any]], bool, Optional[Dict[str, Any]], bool, bool, Optional[str], bool, Optional[int]) -> Any
        ...
        while len(tweets) < count and attempts < max_attempts:
            if page_limit is not None and attempts >= page_limit:
                break
            attempts += 1
    ...
    def _graphql_get(self, operation_name, variables, features, field_toggles=None):
        ...
        profiler = get_profiler()
        with profiler.span("client.graphql_get", operation=operation_name):
    ...
    def _graphql_post(self, operation_name, variables, features=None):
        ...
        profiler = get_profiler()
        with profiler.span("client.graphql_post", operation=operation_name):
            query_id = _resolve_query_id(operation_name, prefer_fallback=True, url_fetch_fn=_url_fetch)
            using_fallback = query_id == FALLBACK_QUERY_IDS.get(operation_name)
            ...
            try:
                ...
                if exc.status_code in (404, 422) and using_fallback:
                    logger.info(
                        "Retrying POST %s with live queryId after %d",
                        operation_name,
                        exc.status_code,
                    )
                    _invalidate_query_id(operation_name)
    ...
    def _api_request(self, url, method="GET", body=None):
        ...
        with profiler.span("client.api_request", method=method, url_path=url_path):
            ...
            for attempt in range(self._max_retries + 1):
                ...
                profiler.mark("client.api_request.success", attempt=attempt + 1, method=method)
                return parsed
    ...
    def _ensure_client_transaction(self):
        ...
        """
        profiler = get_profiler()
        with profiler.span("client.client_transaction.ensure"):
            if self._ct_init_attempted:
                profiler.mark("client.client_transaction.skip", reason="already_attempted")
                return
            self._ct_init_attempted = True

            with profiler.span("client.client_transaction.cache_load"):
                cache_hit = self._load_ct_cache()
            profiler.mark("client.client_transaction.cache", hit=cache_hit)
            if cache_hit:
                return
            ...
            try:
                ...
                with profiler.span("client.client_transaction.fetch_homepage"):
                ...
                with profiler.span("client.client_transaction.fetch_ondemand"):
                    ondemand_file = cffi_session.get(
                        ondemand_url, headers=ct_headers, timeout=10,
                    )
                with profiler.span("client.client_transaction.construct"):
                    self._client_transaction = ClientTransaction(
                        home_page_response=home_page_response,
                ...
                with profiler.span("client.client_transaction.update_features"):
                    _update_features_from_html(home_page.text)
                with profiler.span("client.client_transaction.save_cache"):
                    self._save_ct_cache(home_page.text, ondemand_file.text)
            except Exception as exc:
                profiler.mark(
                    "client.client_transaction.error",
