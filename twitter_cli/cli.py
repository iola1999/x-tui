...
from .profiling import finish_active_profiler, get_profiler, start_command_profiler
...
def _get_client(config=None, quiet=False):
    ...
    with profiler.span("cli.get_client.total", quiet=quiet):
        if not quiet:
            console.print("\n🔐 Getting Twitter cookies...")
        with profiler.span("cli.get_client.cookies"):
            cookies = get_cookies()
        rate_limit_config = (config or {}).get("rateLimit")
        with profiler.span("cli.get_client.client_init"):
...
def cli(ctx, verbose, compact):
    ...
    start_command_profiler(
        sys.argv[1:],
        module_import_ms=0.0,
    )
    profiler = get_profiler()
    profiler.mark("cli.callback.enter", verbose=verbose, compact=compact)
    if ctx.invoked_subcommand:
        profiler.set_field("command", ctx.invoked_subcommand)
    ensure_utf8_streams()
    _setup_logging(verbose)
    ctx.ensure_object(dict)
    ctx.obj["compact"] = compact
    ctx.call_on_close(lambda: finish_active_profiler(status="ok"))
...
@click.option("--pages", type=click.IntRange(min=1), default=None, help="Max timeline pages to fetch.")
@click.option("--full-text", is_flag=True, help="Show full reply text in table output.")
@structured_output_options
@click.pass_context
def tweet(ctx, tweet_id, max_count, pages, full_text, as_json, as_yaml):
    # type: (Any, str, int, Optional[int], bool, bool, bool) -> None
...
@structured_output_options
@click.pass_context
def show(ctx, index, max_count, pages, full_text, output_file, as_json, as_yaml):
    # type: (Any, int, Optional[int], Optional[int], bool, Optional[str], bool, bool) -> None
...
def daemon_command():
    # type: () -> None
    """Run a JSONL request loop for resident-process integrations."""
