...
def extract_from_browser() -> Tuple[Optional[Dict[str, str]], List[str]]:
    ...
    with profiler.span("auth.extract.in_process"):
        cookies, diag = _extract_in_process()
    all_diagnostics.extend(diag)
    if cookies:
        profiler.mark("auth.extract.source", source="in_process")
    ...
    with profiler.span("auth.extract.subprocess"):
        cookies, diag = _extract_via_subprocess()
    all_diagnostics.extend(diag)
    if cookies:
        profiler.mark("auth.extract.source", source="subprocess")
    if not cookies:
...
def get_cookies() -> Dict[str, str]:
    ...
    try:
        with profiler.span("auth.verify"):
