...
class TestPaginationBehavior:
    ...
    def test_fetch_timeline_stops_after_page_limit(self):
        client = TwitterClient.__new__(TwitterClient)
        client._request_delay = 0.0
        client._max_count = 200
        ...
        def _parse_timeline_response(data, get_instructions):
            if data["page"] == 1:
                return [MagicMock(id="tweet-1"), MagicMock(id="tweet-2")], "cursor-next"
            return [MagicMock(id="tweet-3")], None

        client._graphql_get = _graphql_get

        with patch("twitter_cli.client.parse_timeline_response", side_effect=_parse_timeline_response):
            tweets = client._fetch_timeline("TweetDetail", 20, lambda data: data, page_limit=1)

        assert [tweet.id for tweet in tweets] == ["tweet-1", "tweet-2"]
        assert len(calls) == 1
...
class TestFetchTweetHead:
    def test_fetch_tweet_head_uses_tweet_result_operation(self):
        client = TwitterClient.__new__(TwitterClient)
        captured = {}
        ...
        def _graphql_get(operation_name, variables, features, field_toggles=None):
            ...
            return {
                "data": {
                    "tweetResult": {
                        "result": copy.deepcopy(TestParseTweetResult.SAMPLE_TWEET_RESULT),
                    }
                }
            }
        ...
        tweet = client.fetch_tweet_head("1234567890")

        assert tweet.id == "1234567890"
        assert captured["operation_name"] == "TweetResultByRestId"
        assert captured["variables"]["tweetId"] == "1234567890"
