...
def test_cli_tweet_accepts_shared_url_with_query(monkeypatch) -> None:
    class FakeClient:
        def fetch_tweet_detail(self, tweet_id: str, max_count: int, page_limit=None):
            assert tweet_id == "12345"
            assert max_count == 50
            assert page_limit is None
            return []
...
def test_cli_tweet_passes_page_limit(monkeypatch) -> None:
    class FakeClient:
        def fetch_tweet_detail(self, tweet_id: str, max_count: int, page_limit=None):
            assert tweet_id == "12345"
            assert max_count == 50
    ...
    result = runner.invoke(cli, ["tweet", "12345", "--pages", "1"])
...
def test_show_happy_path(monkeypatch, tmp_path, tweet_factory):
    ...
    class FakeClient:
        def fetch_tweet_detail(self, tweet_id, count, page_limit=None):
            assert tweet_id == "42"
            assert page_limit is None
            return [tw]
